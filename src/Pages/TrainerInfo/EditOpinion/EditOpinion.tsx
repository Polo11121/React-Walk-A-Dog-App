...
export const EditOpinion = () => {
  ...
  const onSuccess = () => {
    queryClient.invalidateQueries(["opinion", `${params.id}`]).then(() => goBack()); // eslint-disable-next-line react-hooks/rules-of-hooks
    useCustomToast("Zedytowano opinie!");
  };

  const isButtonDisabled = (valueText === opinion?.review && valueRating === opinion?.points) || !valueText.trim();

  const { mutate } = useEditOpinion(onSuccess)

  const editOpinion = () => mutate({id: +`${params.id}`, points: valueRating || 0, review: valueText});
